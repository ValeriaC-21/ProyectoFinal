...
public final class MongoDataStore {
    ...
    public List<Usuario> cargarUsuarios() {
        ...
        for (Document documento : usuariosCollection().find().sort(Sorts.ascending("cedulaId"))) {
            usuarios.add(new Usuario(
                    obtenerString(documento, "cedulaId"),
                    obtenerString(documento, "nombre"),
                    normalizarCargo(obtenerString(documento, "cargo")),
                    obtenerString(documento, "telefono"),
                    obtenerString(documento, "password")
            ));
        }
        return usuarios;
    }
    ...
    /**
     * Obtiene un campo del documento como String, sin lanzar ClassCastException
     * si el valor quedó guardado en Mongo con otro tipo (Integer, Long, Double, Boolean, etc.).
     * Devuelve null si el campo no existe o es null.
     */
    private String obtenerString(Document documento, String campo) {
    ...
    public List<Mantenimiento> cargarMantenimientos() {
        ...
        for (Document documento : mantenimientosCollection().find().sort(Sorts.ascending("id"))) {
            Mantenimiento mantenimiento = new Mantenimiento(
                    obtenerString(documento, "idEquipo"),
                    obtenerString(documento, "tipo"),
                    obtenerString(documento, "descripcion"),
                    obtenerString(documento, "tecnicoResponsable"),
                    parseLocalDate(obtenerString(documento, "fecha"))
            );
            String id = obtenerString(documento, "id");
    ...
    public List<Falla> cargarFallas() {
        ...
        for (Document documento : fallasCollection().find().sort(Sorts.ascending("id"))) {
            Falla falla = new Falla(
                    obtenerString(documento, "idEquipo"),
                    obtenerString(documento, "descripcion"),
                    obtenerString(documento, "usuarioReporta"),
                    obtenerString(documento, "cedulaUsuarioReporta"),
                    parseLocalDate(obtenerString(documento, "fechaReporte"))
            );
            String id = obtenerString(documento, "id");
            falla.setId(id);
            String estado = obtenerString(documento, "estadoFalla");
            if (estado != null) {
    ...
    public void guardarFalla(Falla falla) {
        Document documento = new Document("id", falla.getId())
                ...
                .append("cedulaUsuarioReporta", falla.getCedulaUsuarioReporta())
    ...
    private DatosEquipo construirEquipo(Document documento) {
        ...
        String tipo = obtenerString(documento, "tipo");
        int cantidad = documento.getInteger("cantidad", 0);
        String clase = obtenerString(documento, "clase");
        String subtipo = obtenerString(documento, "subtipo");
        LocalDate fechaIngreso = parseLocalDate(obtenerString(documento, "fechaIngreso"));
        ...
        String estado = obtenerString(documento, "estado");
