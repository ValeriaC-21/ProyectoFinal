...
public class GestionMantenimientoView extends BaseSecuredView {
    ...
    private final TextField buscarId = new TextField("Buscar por Id de mantenimiento...");

    // Solo el Administrador y el Técnico de Mantenimiento gestionan mantenimientos
    @Override
    protected List<String> getRolesPermitidos() {
        return List.of(UiUtils.ROL_ADMINISTRADOR, UiUtils.ROL_TECNICO);
    }
    ...
    public GestionMantenimientoView() {
        ...
        buscarId.setWidthFull();
        buscarId.setPlaceholder("Ej: M-0001");
        buscarId.addValueChangeListener(event -> aplicarFiltroId());
        ...
        cabecera.add(buscarId, nuevo);
        cabecera.expand(buscarId);
    ...
    private void aplicarFiltroId() {
        String texto = buscarId.getValue() == null ? "" : buscarId.getValue().trim().toLowerCase();
        dataProvider.clearFilters();
        dataProvider.setFilter(m -> m.getId() != null && (texto.isBlank() || m.getId().toLowerCase().contains(texto)));
    }
    ...
    private void abrirDialogoMantenimiento() {
        ...
        cedulaTec.setValueChangeMode(ValueChangeMode.EAGER);
        cedulaTec.addValueChangeListener(e -> {
            String val = e.getValue() == null ? "" : e.getValue().trim();
            if (val.isEmpty()) {
                cedulaTec.setInvalid(false);
                return;
            }
            if (val.length() != 10 || !val.matches("\\d+")) {
                cedulaTec.setErrorMessage("La cédula debe contener exactamente 10 dígitos numéricos.");
                cedulaTec.setInvalid(true);
            } else if (Sistema.gestorUsuario.buscarPorCedula(val) == null) {
                cedulaTec.setErrorMessage("No existe un usuario registrado con esta cédula.");
                cedulaTec.setInvalid(true);
            } else {
                cedulaTec.setInvalid(false);
            }
        ...
        guardar.addClickListener(event -> {
            try {
                ...
                if (cedula.length() != 10 || !cedula.matches("\\d+")) {
                    throw new DatoInvalidoException("La cédula del técnico debe contener exactamente 10 dígitos numéricos.");
                }
