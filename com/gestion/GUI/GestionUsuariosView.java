...
public class GestionUsuariosView extends BaseSecuredView {
    ...
    public GestionUsuariosView() {
        ...
        // ---- BARRA DE BÚSQUEDA (por número de cédula) ----
        buscarNombre.setPlaceholder("Buscar por cédula...");
    ...
    private void aplicarFiltros() {
        ...
        dataProvider.setFilter(user -> user.getCedulaId() != null && user.getCedulaId().toLowerCase(Locale.ROOT).contains(texto));
    }
    ...
    private void abrirDialogoUsuario(Usuario usuarioAEditar) {
        ...
        cedulaField.addValueChangeListener(e -> {
            if (usuarioAEditar != null) {
                return; // la cédula ya está validada y bloqueada al editar
            }
        ...
        nombreField.addValueChangeListener(e -> {
            ...
            } else if (!val.matches(REGEX_NOMBRE)) {
        ...
        btnGuardar.addClickListener(event -> {
            ...
            if (nombreField.isInvalid() || (usuarioAEditar == null && cedulaField.isInvalid()) || telefonoField.isInvalid()) {
                Notification.show("[AVISO] Por favor, complete todos los campos respetando los formatos exigidos.", 4000, Notification.Position.MIDDLE)
                        .addThemeVariants(NotificationVariant.LUMO_ERROR);
                return;
            }
